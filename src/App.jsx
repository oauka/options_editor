...
const ProductOptionsEditor = () => {
  ...
  useEffect(() => {
    ...
    script.src = 'https://pl28524393.effectivegatecpm.com/53/b1/05/53b105e000f3fd46e90cf9fd893d264c.js';
    script.async = true;
  ...
  const exportImage = async () => {
    // 미리보기 영역이 없으면 먼저 미리보기 모드로 전환
    if (!document.getElementById('preview-area')) {
      setPreviewMode(true);
      // 미리보기가 렌더링될 시간을 주고 다시 호출
    ...
    // domtoimage 로드 (html2canvas보다 안정적)
    ...
    try {
      ...
      // PNG로 변환
      const blob = await window.domtoimage.toBlob(previewElement, {
        bgcolor: backgroundColor,
        quality: 1,
        width: previewElement.offsetWidth * 2,
      ...
      alert('이미지 생성에 실패했습니다.\n\n대신 "HTML 다운로드"를 사용하시면 HTML 파일 내에서 이미지로 저장할 수 있습니다.');
    }
  };
...
${opt.specs.map(spec => `          <div class="spec-item" style="color: ${opt.specsColor};"><div class="spec-content" style="text-align: ${opt.specsAlign}; transform-origin: ${opt.specsAlign === 'left' ? 'left' : opt.specsAlign === 'right' ? 'right' : 'center'};">${spec.text}</div></div>`).join('\n')}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* 왼쪽 광고 배너 영역 */}
      <div className="w-48 flex-shrink-0 bg-gray-100 p-4" style={{ position: 'relative', zIndex: 1 }}>
                    ...
                    <div className="space-y-3 p-3 bg-blue-50 border border-blue-200 rounded">
                      ...
                      <div>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <div>
                              <label className="block text-xs text-gray-700 mb-1">크기: {option.circleOverlay.innerImage?.scale || 100}%</label>
                            ...
                            <div>
                              <label className="block text-xs text-gray-700 mb-1">가로: {Math.round(option.circleOverlay.innerImage?.position?.x || 50)}%</label>
                              <div className="flex gap-1 items-center">
                                <input
                                  type="range"
                                  min="0"
                            ...
                            <div>
                              <label className="block text-xs text-gray-700 mb-1">세로: {Math.round(option.circleOverlay.innerImage?.position?.y || 50)}%</label>
                              <div className="flex gap-1 items-center">
                                <input
                                  type="range"
                                  min="0"
                  ...
                  {option.textBox.enabled && (
                    <div className="space-y-2 p-3 bg-green-50 border border-green-200 rounded">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <div className="flex items-center gap-1">
                          <label className="text-xs text-gray-700">글자 크기:</label>
                          <select
